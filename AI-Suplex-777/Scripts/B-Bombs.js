// BBomb.js – 7‑7‑7 Edition
// Prompts for focus, cycle, week, title, description, product potential, content.
// Saves to AI-Suplex-777/B-Bombs/Cycle X/Week Y/

module.exports = async (quickAdd) => {
    const { quickAddApi, app } = quickAdd;
    const vault = app.vault;

    // Helper: run 3lm learn to extract lesson from capture
    function exec3lm(command) {
        try {
            const { execSync } = require("child_process");
            const vaultPath = app.vault.adapter.basePath;
            const cwd = vaultPath + "/AI-Suplex-777";
            return execSync(`node Tools/3lm.js ${command}`, {
                encoding: "utf8",
                timeout: 15000,
                stdio: "pipe",
                cwd,
            });
        } catch (e) {
            console.error("3lm " + command + " error: " + e.message);
            return null;
        }
    }

    async function getFocuses() {
        const focusesPath = "AI-Suplex-777/Focuses.md";
        if (!(await vault.adapter.exists(focusesPath))) {
            return ["ai-engineering", "wqr", "freelance", "digital-products", "content-creation"];
        }
        const content = await vault.adapter.read(focusesPath);
        const match = content.match(/^---\n([\s\S]*?)\n---/);
        if (!match) return [];
        const frontmatter = match[1];
        const nameMatches = [...frontmatter.matchAll(/name:\s*(\S+)/g)];
        return nameMatches.map(m => m[1]);
    }

    function capitalizeFocus(focus) {
        if (!focus) return '';
        const acronyms = { 'ai': 'AI', 'wqr': 'WQR', 'api': 'API', 'rbz': 'RBZ', 'zimra': 'ZIMRA', 'potraz': 'POTRAZ' };
        return focus.split('-').map(part => {
            const lower = part.toLowerCase();
            return acronyms[lower] || (part.charAt(0).toUpperCase() + part.slice(1).toLowerCase());
        }).join('-');
    }

    async function ensureFolder(folderPath) {
        if (!(await vault.adapter.exists(folderPath))) {
            await vault.createFolder(folderPath);
        }
    }

    const focusOptions = await getFocuses();
    const focus = await quickAddApi.suggester(focusOptions, focusOptions, "Select focus area");
    const displayFocus = capitalizeFocus(focus);

    const cycleOptions = [1,2,3,4,5,6,7];
    const cycle = await quickAddApi.suggester(cycleOptions, cycleOptions, "Select cycle (1-7)");
    const weekOptions = [1,2,3,4,5,6,7];
    const week = await quickAddApi.suggester(weekOptions, weekOptions, "Select week (1-7)");

    const title = await quickAddApi.inputPrompt("B‑Bomb Title");
    const description = await quickAddApi.inputPrompt("Description");
    const productPotential = await quickAddApi.inputPrompt("Product Potential (optional)");
    const bombContent = await quickAddApi.inputPrompt("B‑Bomb content", null, { multiline: true });

    const now = new Date();
    const formattedDate = now.toISOString().slice(0,10);
    const [year, month, day] = formattedDate.split("-");
    const timeForFilename = now.toISOString().slice(11,16).replace(":", "");
    const safeTitle = title.toLowerCase().replace(/[^\w\s-]/g, '').replace(/\s+/g, '-').slice(0,50);
    const fileName = `${year}-${month}-${day}-${timeForFilename}-${focus}-${safeTitle}.md`;
    const folderPath = `AI-Suplex-777/B-Bombs/Cycle ${cycle}/Week ${week}`;
    await ensureFolder(folderPath);
    const filePath = `${folderPath}/${fileName}`;

    const trackerLink = `[[${displayFocus} Tracker]]`;
    const mocLink = `[[${displayFocus} MOC]]`;

    const frontmatter = `---
tags:
  - b-bomb
  - ${focus}
  - #7/${cycle}/${week}
date: ${formattedDate}
time: ${now.toTimeString().slice(0,5)}
focus: ${focus}
cycle: ${cycle}
week: ${week}
product_potential: "${productPotential || ''}"
---

# 💣 B-BOMB: ${title}

**Author:** (User's Name)  
**Status:** Ready for use

## Description
${description || ''}

---

## 💣 B-Bomb content
${bombContent || '*(Content to be added)*'}

---

## Insights
- 

## Applications
*Where can this B‑Bomb be applied?*
- 

## Quality Assessment
**Innovation Quotient:** /10  
**Completeness:** /10  
**Reusability:** /10  
**Documentation:** /10

## Integration Points
- **[[Command Center]]**
- **${trackerLink}**
- **${mocLink}** 
- **Created during session:** 
- **Template used:** B‑Bomb.md

## Next Steps
*How can this B‑Bomb be improved or expanded?*
- 

## Version History
- **v1.0** – ${formattedDate} – Initial creation

---
**B‑Bomb added to arsenal.**
`;

    await vault.create(filePath, frontmatter);
    const file = vault.getAbstractFileByPath(filePath);
    await app.workspace.openLinkText(file.path, "", false);
    new Notice(`B‑Bomb created: ${fileName}`, 3000);

    // 3lm Quick Capture — extract lesson immediately
    try {
        new Notice("🧠 Extracting lesson...", 1500);
        exec3lm("learn");
        exec3lm("index");
        new Notice("✅ Lesson captured", 1500);
    } catch (e) {
        console.error("3lm capture error: " + e.message);
    }
};